...
* `adomik-nob.js` - This is your partner module file, by default it contains a template divided into multiple sections which need to be completed.
* `adomik-nob-validator.js` - This is the validator file for the configuration object that will be passed into your module.
* `adomik-nob-exports.js` - A file that contains all of the modules exports (i.e. any functions that need to be exposed to the outside world).
...
1. <b>Complete the adomik-nob.js file </b>
    * adomik-nob.js is where all of your adapter code will live.
...
2. <b>Complete the adomik-nob-validator.js file</b>
...
3. <b>Complete the adomik-nob-exports.js file</b>
    * This file will contain any functions that need to be exported or exposed to the outside world. Things like render functions, custom callbacks, etc. Any legacy render functions will also need to be exposed here. Anything added to the `shellInterface.AdomikNob` will be accessible through `window.headertag.AdomikNob`
...
for (var i = 0; i < 10; i++) {
    ...
    if (w.headertag) {
        try {
            w.headertag.AdomikNob.render(document, %%PATTERN:TARGETINGMAP%%, '%%WIDTH%%', '%%HEIGHT%%');
            break;
        } catch (e) {
            continue;
        }
...
In this section you will be filling out the adomik-nob.js, adomik-nob-exports.js, and the adomik-nob-validator.js files to create your module.

### Step 0: Config Validation (`adomik-nob-validator.js`)
...
### Step 1: Partner Configuration (`adomik-nob.js`)
...
### Step 2: Generate Request URL (`adomik-nob.js`)
...
### Step 3: Response Callback (`adomik-nob.js`)
...
### Step 4: Parsing and Storing Demand (`adomik-nob.js`)
...
### Step 5: Rendering (`adomik-nob.js`)
...
### Step 6: Exports (`adomik-nob-exports.js`)
...
```javascript
shellInterface.AdomikNob = { //shell interface is the window variable that is accessable through the window object, currently this will always be window.headertag
    render: SpaceCamp.services.RenderService.renderDfpAd.bind(null, 'AdomikNob')
};
```
...
```javascript
if (__directInterface.Layers.PartnersLayer.Partners.AdomikNob) {
    shellInterface.AdomikNob = shellInterface.AdomikNob || {};
    shellInterface.AdomikNob.adResponseCallback = __directInterface.Layers.PartnersLayer.Partners.AdomikNob.adResponseCallback;
}
```
